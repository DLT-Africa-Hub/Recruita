...
export function getStreamClient(): StreamClient {
    ...
    const streamSecret = process.env.STREAM_API_SECRET || process.env.STREAM_SECRET_KEY;

    if (!streamApiKey || !streamSecret) {
        throw new Error('STREAM_API_KEY and STREAM_SECRET_KEY environment variables must be configured');
    }
 ...
 */
export function generateStreamToken(userId: string, expirationInSeconds: number = 60 * 60, callCids: string[] = []): string {
