...
const AdminCalendlyIntegration = () => {
  ...
  const setPublicLinkMutation = useMutation({
    mutationFn: async (link: string) => {
      await adminApi.setCalendlyPublicLink(link);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminCalendlyStatus'] });
  ...
  const handleDisconnect = () => {
    if (
      window.confirm(
        'Are you sure you want to disconnect your Calendly account? Candidates will no longer be able to schedule interviews through Calendly for admin-managed jobs.'
      )
    ) {
          ...
          <div className="flex items-center gap-[12px]">
            ...
            <div>
              ...
              <p className="text-[13px] text-[#1C1C1C80]">
                {isConnected
                  ? 'Candidates can schedule interviews via Calendly for admin-managed jobs'
                  : 'Connect your Calendly account to enable interview scheduling for admin-managed jobs'}
              </p>
            </div>
          </div>
          {isConnected ? (
        ...
        {!isConnected && (
          <div className="p-[16px] rounded-[12px] bg-blue-50 border border-blue-200">
            <p className="text-[13px] text-blue-800">
              ...
              directly for admin-managed jobs. You can also set a public
              Calendly link for easy sharing.
            </p>
          </div>
        )}
      </div>
...
export default AdminCalendlyIntegration;
